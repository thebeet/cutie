...
describe('Points Intersect', () => {
    ...
    test('octree test intersect box', () => {
        let sum = 0;
        octree.intersect(new THREE.Box3(
    ...
    test('octree test intersect ray', () => {
        const ray = new THREE.Ray(
            new THREE.Vector3(-1, -1, -1),
            new THREE.Vector3(1, 1, 1).normalize(),
        );
        let sum = 0;
        octree.intersectRay(ray, 0.001, () => {
            sum++;
        });
        expect(sum).toBe(100);

        sum = 0;
        octree.intersectRay(ray, 0.0081649, () => {
            sum++;
        });
        expect(sum).toBe(100);

        sum = 0;
        octree.intersectRay(ray, 0.0081651, () => {
            sum++;
        });
        expect(sum).toBe(100 + 6 * 99);
    });

    const rebuildoctree = Octree.fromSerialization(points.geometry, octree.serialization());
