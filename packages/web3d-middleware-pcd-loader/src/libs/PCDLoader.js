...
class PCDLoader extends Loader {
	...
	parse( data ) {
		...
		const textData = new TextDecoder().decode( data.slice(0, 10000) );
		...
		const position = new Float32Array( PCDheader.points * 3 );
        let pointP = 0;
		const normal = [];
		const color = [];
		const intensity = new Float32Array( PCDheader.points );
        let intensityP = 0;
		const label = [];
		...
		if ( PCDheader.data === 'ascii' ) {
			...
			for ( let i = 0, l = lines.length; i < l; i ++ ) {
				...
				if ( offset.x !== undefined ) {
					...
					position[pointP++] =  parseFloat( line[ offset.x ] ) ;
					position[pointP++] =  parseFloat( line[ offset.y ] ) ;
					position[pointP++] =  parseFloat( line[ offset.z ] ) ;
				...
				if ( offset.intensity !== undefined ) {
					...
					intensity[intensityP++] =  parseFloat( line[ offset.intensity ] ) ;
		...
		if ( PCDheader.data === 'binary_compressed' ) {
			...
			for ( let i = 0; i < PCDheader.points; i ++ ) {
				...
				if ( offset.x !== undefined ) {
					...
					position[pointP++] =  dataview.getFloat32( ( PCDheader.points * offset.x ) + PCDheader.size[ xIndex ] * i, this.littleEndian ) ;
					position[pointP++] =  dataview.getFloat32( ( PCDheader.points * offset.y ) + PCDheader.size[ yIndex ] * i, this.littleEndian ) ;
					position[pointP++] =  dataview.getFloat32( ( PCDheader.points * offset.z ) + PCDheader.size[ zIndex ] * i, this.littleEndian ) ;
				...
				if ( offset.intensity !== undefined ) {
					...
					intensity[intensityP++] =  dataview.getFloat32( ( PCDheader.points * offset.intensity ) + PCDheader.size[ intensityIndex ] * i, this.littleEndian ) ;
		...
		if ( PCDheader.data === 'binary' ) {
			...
			for ( let i = 0, row = 0; i < PCDheader.points; i ++, row += PCDheader.rowSize ) {
				...
				if ( offset.x !== undefined ) {
					...
					position[pointP++] =  dataview.getFloat32( row + offset.x, this.littleEndian ) ;
					position[pointP++] =  dataview.getFloat32( row + offset.y, this.littleEndian ) ;
					position[pointP++] =  dataview.getFloat32( row + offset.z, this.littleEndian ) ;
				...
				if ( offset.intensity !== undefined ) {
					...
					intensity[intensityP++] =  dataview.getFloat32( row + offset.intensity, this.littleEndian ) ;
